...
public class ETHWalletUtils {
    ...
    public static ETHWallet generateMnemonic(String walletName, String pwd) {
        ...
        Log.i("创建钱包的seed",ds.toString());
        Log.i("创建钱包的seed1","get this");
...
//        System.out.println(Arrays.toString(mnemonic.toArray()));
        ...
        Log.i("创建钱包的seed2","get this");

        if (ethWallet != null) {
            Log.i("创建钱包的seed3","ethWallet != null");
    ...
    private static ETHWallet generateWallet(String walletName, String pwd, ECKeyPair ecKeyPair) {
        ...
        try {
            ...
            e.printStackTrace();
            Log.i("创建钱包的seed3", String.valueOf(e));
            return null;
        }
...
//            ToastUtils.showToast(R.string.load_wallet_by_official_wallet_keystore_input_tip);
    ...
    private static boolean createParentDir(File file) {
        ...
        //判断目标文件所在的目录是否存在
        Log.i("创建钱包的seed4", "file.getParentFile().exists()");
    ...
    public static String derivePublicKey(long walletId, String pwd) {
        ETHWallet ethWallet = WalletDaoUtils.ethWalletDao.load(walletId);
        Credentials credentials;
        ECKeyPair keypair;
        ...
        try {
            credentials = WalletUtils.loadCredentials(pwd, ethWallet.getKeystorePath());
            keypair = credentials.getEcKeyPair();
            publicKey = Numeric.toHexStringNoPrefixZeroPadded(keypair.getPublicKey(), 128);
        } catch (CipherException e) {
            e.printStackTrace();
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public static ECKeyPair deriveECKeyPair(long walletId, String pwd) {
        ETHWallet ethWallet = WalletDaoUtils.ethWalletDao.load(walletId);
        Credentials credentials;
        ECKeyPair keypair = null;
        try {
    ...
    public static Credentials deriveCredentials(long walletId, String pwd) {
        ETHWallet ethWallet = WalletDaoUtils.ethWalletDao.load(walletId);
        Credentials credentials = null;
        try {
            credentials = WalletUtils.loadCredentials(pwd, ethWallet.getKeystorePath());
